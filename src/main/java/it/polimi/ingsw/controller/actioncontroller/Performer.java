...
public class Performer implements CharacterCardStrategy {
    ...
    public void useEffect(ActionMessage actionMessage) {
        Student s1 = board.getSchoolByOwner(gameModel.getCurrentPlayer().getNickname()).removeDiningRoomStudent(CharacterColor.valueOf(actionMessage.getParameters().get(0)));
        Student s2 = board.getSchoolByOwner(gameModel.getCurrentPlayer().getNickname()).removeEntranceStudent(CharacterColor.valueOf(actionMessage.getParameters().get(1)));
